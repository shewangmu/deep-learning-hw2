...
def content_loss(content_weight, content_current, content_target):
    ...
    - scalar content loss
    """
#    content_cur_var = Variable(content_current)
#    content_tar_var = Variable(content_target)
#    loss = (content_cur_var-content_tar_var).pow(2).sum()
    loss = (content_current-content_target).pow(2).sum()
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i in range(len(style_layers)):
        feature_gram = gram_matrix(feats[style_layers[i]])
        target_gram = style_targets[i]
        loss += (feature_gram-target_gram).pow(2).sum()*style_weights[i]
    return loss
...
def tv_loss(img, tv_weight):
    ...
    img_nt_w = img[:,:,:,1:]
    img_pr_w = img[:,:,:,:-1]
    loss = ((img_nt_h - img_pr_h).pow(2).sum() + (img_nt_w - img_pr_w).pow(2).sum())*tv_weight
    return loss 
...
def style_transfer(content_image, style_image, image_size, style_size, content_layer, content_weight,
    ...
    for t in range(300):
        ...
        #TODO:Compute loss
        contentLoss = content_loss(content_weight, feats[content_layer], content_target)
        styleLoss = style_loss(feats, style_layers, style_targets, style_weights)
...
if __name__== "__main__":
    ...
    a = style_transfer(**params2)
    plt.plot(range(200), a)
    plt.title("learning loss of curve for the third imige")
